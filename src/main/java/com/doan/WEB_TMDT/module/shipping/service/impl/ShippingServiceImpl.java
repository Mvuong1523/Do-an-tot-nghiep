...
public class ShippingServiceImpl implements ShippingService {
    ...
    private Map<String, Object> callGHNApi(CalculateShippingFeeRequest request) {
        ...
        try {
            ...
            if (leadTimeResponse != null && leadTimeResponse.get("code") != null
                    && leadTimeResponse.get("code").equals(200)) {
                @SuppressWarnings("unchecked")
    ...
    private Integer getProvinceId(String provinceName) {
        try {
            ...
            if (response != null && response.get("code") != null && response.get("code").equals(200)) {
                ...
                provinces.stream().limit(5).forEach(p -> log.warn("  - {}", p.get("ProvinceName")));
            }
    ...
    private boolean matchLocation(String ghnName, String userInput) {
        ...
        // For single digit districts (Quận 1, Quận 2, etc), require exact match with
        // space or end
    ...
    public CreateGHNOrderResponse createGHNOrder(CreateGHNOrderRequest request) {
        try {
            ...
            if (wardCode == null || wardCode.trim().isEmpty()) {
                ...
                log.warn("⚠️ No ward code provided, attempting to get default ward for district: {}",
            ...
            if (response != null && response.get("code") != null && response.get("code").equals(200)) {
                ...
                if (data != null) {
                    ...
                    return GHNOrderDetailResponse.builder()
                            ...
                            .codAmount(data.get("cod_amount") != null ? ((Number) data.get("cod_amount")).doubleValue()
                                    : null)
                            .shippingFee(data.get("total_fee") != null ? ((Number) data.get("total_fee")).doubleValue()
                                    : null)
    ...
    private String getStatusText(String status) {
        ...
        switch (status) {
            ...
            case "money_collect_delivering":
                return "Đang thu tiền người nhận";
            case "delivered":
                return "Đã giao hàng";
            case "delivery_fail":
    ...
    public Map<String, Object> fixAllWardNames() {
        ...
        try {
            ...
            List<com.doan.WEB_TMDT.module.order.entity.Order> orders = orderRepository.findAll().stream()
            ...
            for (com.doan.WEB_TMDT.module.order.entity.Order order : orders) {
                try {
                    ...
                    if (wardOpt.isPresent()) {
                        ...
                        failCount++;
                        String error = "Order " + order.getOrderCode() + ": Ward code " + order.getWard()
                                + " not found in district " + districtId;
                        errors.add(error);
                        log.warn("⚠️ {}", error);
                    }
