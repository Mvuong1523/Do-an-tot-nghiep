...
export default function ImportDetailPage() {
  ...
  const loadOrderDetail = async () => {
    try {
      ...
      if (response.success) {
        ...
        router.push('/employee/warehouse/import/list')
      }
    } catch (error) {
  ...
  return (
    <div>
      <div className="mb-6">
        <Link
          href="/employee/warehouse/import/list"
      ...
      {/* Permission notice for non-warehouse staff */}
      {!canApprove && order.status === 'CREATED' && (
        <div className="mb-6 bg-amber-50 border border-amber-200 rounded-lg p-4">
          <div className="flex items-start">
            <FiFileText className="text-amber-500 mt-0.5 mr-3" size={20} />
            <div>
              <h3 className="text-sm font-medium text-amber-900">Quyền hạn của bạn</h3>
              <p className="text-sm text-amber-700 mt-1">
                Bạn có thể xem chi tiết phiếu nhập, nhưng không thể hoàn thiện phiếu nhập. Chỉ nhân viên kho mới có quyền này.
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white rounded-lg shadow-sm overflow-hidden">
          <div className="p-6 border-b border-gray-200">
            ...
            {order.status === 'CREATED' && canApprove && (
                          ...
                          {Array.from({ length: item.quantity }).map((_, idx) => (
                            <div key={idx} className="flex items-center space-x-2">
                              <input
                                ...
                                className="flex-1 px-2 py-1 text-xs border border-gray-300 rounded focus:ring-2 focus:ring-blue-500 focus:border-transparent font-mono"
                              />
                              <button
        ...
        {order.status === 'CREATED' && canApprove && (
          <div className="bg-white rounded-lg shadow-sm p-6">
            <button
              ...
              className="w-full bg-blue-600 text-white px-6 py-3 rounded-lg hover:bg-blue-700 transition-colors flex items-center justify-center space-x-2 font-semibold disabled:bg-gray-400 disabled:cursor-not-allowed"
            >
              {isCompleting ? (
                <>
